...
public class rentingDetailDAO implements Serializable {
    ...
    public boolean UpdateCarRentalExpiresStatus(String carID, Date currentDate) throws NamingException, SQLException {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DBHelpers.makeConnection();
    ...
    public void loadOrderByEmail(int rentingID) throws NamingException, SQLException {
        ...
        try {
            con = DBHelpers.makeConnection();
            if (con != null) {
                String sql = "SELECT rentingID,carID,quantity,price,rentalDate,returnDate,status "
                ...
                while (rs.next()) {
                    ...
                    listRentingDetail.add(new rentingDetailDTO(rs.getInt("rentingID"),rs.getString("carID"), rs.getFloat("price"),
